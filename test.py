...
for i, (q, opts) in enumerate(questions):
    ...
    if answer is None:
        continue
    if "벼락치기" in answer or "복습" in answer:
        scores["모범생 3000 ✏️"] += 2
    if "급식" in answer or "밥" in answer:
        scores["급식 요정 🍱"] += 2
    if "복도" in answer or "댄스" in answer:
        scores["복도 아이돌 🎤"] += 2
        scores["급발진 인싸 🚀"] += 1
    if "게임" in answer or "넷플릭스" in answer:
        scores["숨은 게이머 🎮"] += 2
    if "자고" in answer or "잠" in answer:
        scores["잠꾸러기 고라니 😴"] += 2
    if "질문" in answer or "공부" in answer:
        scores["지식 폭격기 📚"] += 2
    if "달리기" in answer or "체육" in answer:
        scores["체육관 파이터 🥊"] += 2
    if "민원" in answer or "환경" in answer:
        scores["환경 지키미 🌱"] += 2
